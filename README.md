...
| 窗口模式 | 在任务栏子窗口与悬浮窗口之间切换；两种模式均支持横向/竖向布局和 70%-125% 显示比例 |
| 自动隐藏 | 可在所有媒体会话均停止播放时隐藏；悬浮窗口支持桌面边缘自动折叠、提示线和平滑动画 |
...
5. 右键播放器或托盘图标，选择“打开详细设置...”集中配置开机启动、组件、布局、外观和交互。
...
| 将悬浮窗口拖到桌面边缘 | 启用“桌面边缘自动折叠”后自动收起；将鼠标移到提示线附近可展开 |
...
- 自动检查更新只请求公开的 `latest.json` 版本清单，不上传媒体信息、设备信息或用户设置；此功能可以在设置中关闭。
